...
| `MprisWidget.qml` | Media player (current track) | MprisPopup |
...
| `MprisPopup.qml` | Media player (details + cava visualization) | CavaMonitor |
| `GenshinPopup.qml` | Genshin details, manual refresh, check-in | GenshinMonitor, `scripts/genshin_stats.py` |
| `AudioMixerPopup.qml` | Audio mixer | PipeWire |
...
| File | What it stores |
|------|----------------|
| `config.json` | Bar settings (visibility, widget order, timer sound) |
