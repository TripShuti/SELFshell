...
* **Daemon:** optional `kcd` (`AUR kcd-bin`, `systemctl --user enable --now kcd`, LAN-only `1716/udp+tcp` `1739:1764/tcp`, no KDE stack, 0% idle when all devices connected). Config `~/.config/kcd/kcd.toml` (`download_dir`, `sftp.mount_dir`, `tcp_port 1716` — all read live, no hardcoded paths). `kcd --version` check → `installed`, `kcd devices --json` 30s poll + `kcd watch --json` live (battery, device.connected, notification, clipboard, sftp, share.progress). `primaryDeviceId/name` from watch `device.connected`/`battery` + poll `connected`/`state`, `isReachable` from `connected` or battery, `lastClipboard`/`sftpVolumes`/`sftpMountPoint` from watch.
