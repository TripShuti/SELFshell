...
- **Shared empty states** — new `core/EmptyHint` (icon + text) for clipboard/launcher/mpris/notifications placeholders. No behavior changes.
...
- **Brightness jumps to intermediate values on slow DDC** — the 15-unit sub-stepping sent ~6 transactions per 100→10 drag while polls picked up the intermediate hardware states, so the slider snapped back to 90/85. Now optimistic display plus a single coalesced write per drag pause (stale poll responses are dropped while a write is pending).
- **Control Center probes reverted to startup behavior** — the lazy warmup + poll reshaping caused hyprsunset resets and sluggish brightness on real hardware. `ControlPopup` and `ReadingTempSection` are back to the pre-audit behavior (probes at shell startup); the ddcutil slowness (~3s per transaction on this bus) is a hardware/driver property, not something the shell can fix by rescheduling.
...
- **System settings section with power profiles** — new Settings → System section: power profile selector (`Performance`/`Balanced`/`Power saver` via `powerprofilesctl`, no root needed, works on both `amd_pstate` and `intel_pstate`) with live status (active profile, governor/EPP readout) plus an `Auto power-saver on battery` toggle (`autoPowerSaver` in `config.json`): drops to `power-saver` at low battery (≤15%, same hysteresis as the low-battery toast) and restores the manual profile on charge. The segment shows only profiles parsed from `powerprofilesctl list` (once at startup, refreshed on `set` failure). New `services/PowerProfileService.qml` singleton owns the state; `install.sh` installs/enables `power-profiles-daemon`, `selfshell doctor` checks it. A `Monitoring` card shows CPU temp/frequency, memory and disk usage from the new `scripts/sysinfo.py` (read-only `/sys`+`/proc`, refreshed every 5s while Settings is open).
- **Pacman/AUR updates in Settings → System** — new `Updates` card: status line (counts, split official/AUR only when AUR updates exist, plus total download size and last-checked age), up to 30 package rows (`name old → new` + repo tag) and `Check for updates` / `Update all` buttons. New `services/PacmanService.qml` singleton owns the state (cached in `data/updates.json`, survives restarts): the expensive check (`scripts/pacman_updates.py` — `checkupdates` for official repos, `yay`/`paru -Qua` for AUR, enriched with repo/description/size) runs only at startup when the cache is older than 24h, once a day via timer, or manually — opening Settings never touches pacman. `Update all` closes Settings and opens `kitty` with `scripts/pacman_upgrade.sh` (`yay -Syu` or `sudo pacman -Syu`, password asked in that terminal, window floats centered 900×600 via a `selfshell-upgrade-float` windowrule matching its title and is explicitly focused after launch via `Hyprland.dispatch`, classic `hyprctl dispatch` syntax does not parse on Lua-based Hyprland); the wrapper signals completion through a sentinel file so the list re-checks itself into `Up to date`. `install.sh` gains `pacman-contrib`, `selfshell doctor` checks `checkupdates`/`fakeroot`/AUR helper; `SetButton` gains an optional `disabled` state.
...
- **MprisPopup destruction warning** — `Component.onDestruction: Qt.callLater(function() { if (root) ... })` threw `ReferenceError: root is not defined` (the guard can't help: the identifier itself doesn't resolve in a destroyed scope). Removed like the identical case in `MprisWidget`; the 2s poller already covers player re-election.
- **IdleManager file-watch crash risk** — `control-state.json` was watched with `watchChanges: true` while `ControlPopup` saves it atomically via `setText` (same use-after-free in the FileView watcher that already disabled watching in `AppConfig`/`PaletteService`/`AudioEq` on Quickshell 0.3.0). Watching is off now; `ControlPopup.saveState()` pushes an explicit `refreshCaffeine()` into the `IdleManager` singleton instead (`shell.qml → Bar → ControlPopup` required-property wiring, `Svc`-suffixed id to avoid the self-binding loop) — same on-demand pattern as `PaletteService`.
- **GenshinMonitor stall after failed sync** — a background sync with an empty response or unparsable JSON returned before starting `mainTimer`, so the local resin recalc and daily/high-resin checks stayed dead until a manual refresh or widget re-toggle. The startup guard is now a shared `_ensureMainTimer()` helper called on every background outcome.
