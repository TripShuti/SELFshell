...
- **SelfTrack timeline zoom** — `popups/SelfTrackPopup.qml` map-like dynamic zoom: wheel zooms 24h→1min around the cursor, click jumps into an hour / recenters when zoomed, `‹ ›` pan by half window, `24h` reset, live window-range label, adaptive ticks (hours → quarters → seconds); zoom resets on date change.
