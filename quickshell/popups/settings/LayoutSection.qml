// ============================================================
// settings/LayoutSection.qml — порядок віджетів: drag-and-drop
// між пігулками, ввімкнення/вимкнення. Прихована пігулка
// (cfg.*PillEnabled) показується притемненою.
// ============================================================
import QtQuick
...
Item {
  ...
  // - після коміту ac.saveToFile() записує новий config.json
  //
    ...
    function lastItemEndX() {
      ...
      for (var i = chipLayout.length - 1; i >= 0; i--) {
        if (Math.floor((chipLayout[i].y - 4) / (chipH + spacing2)) === tr)
          return chipLayout[i].x + (ac.isSep(allNames[i]) ? 6 : chipW) + spacing2
      }
